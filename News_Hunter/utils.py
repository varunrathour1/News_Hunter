...
def summarize_with_anthropic_news_script(api_key: str, headlines: str) -> str:
    # Note: 'def' ke neeche ek hi level ka indent hona chahiye (usually 4 spaces)
    logging.info(f"summarize_with_anthropic_news_script called with headlines (first 500 chars): {headlines[:500]}...")
    """
...
"""
    ...
    try:
        ...
        logging.info(f"Anthropic raw response (first 500 chars): {response.content[:500]}...")

        return response.content
    except Exception as e:
        logging.error(f"Error in summarize_with_anthropic_news_script: {e}", exc_info=True)
